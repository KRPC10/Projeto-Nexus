...
# =============================================================
# 5. LOOP PRINCIPAL (COM ATIVAÇÃO MANUAL E FILTRO DE CONFIANÇA)
# =============================================================
...
while True:
    ...
    # =========================================================
    # ESTADO DE ESPERA: O DOMÍNIO FOI DETECTADO E AGUARDA SUA CONFIRMAÇÃO
    # =========================================================
    if dominio_armado:
        # Efeito visual cinematográfico: escurece a tela para focar na mensagem
        overlay = img.copy()
        cv2.rectangle(overlay, (0, 0), (1280, 720), (0, 0, 0), -1)
        img = cv2.addWeighted(overlay, 0.6, img, 0.4, 0) 
        
        cv2.putText(img, f"ALVO TRAVADO: {dominio_armado}", (250, 300), cv2.FONT_HERSHEY_DUPLEX, 1.5, (0, 255, 255), 3)
        cv2.putText(img, "APERTE [ ENTER ] PARA CAPTURAR A TELA", (230, 380), cv2.FONT_HERSHEY_DUPLEX, 1.0, (0, 255, 0), 2)
        cv2.putText(img, "Aperte [ C ] para cancelar", (450, 430), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)
        
        cv2.imshow("Projeto Nexus - Batalha Cinematografica", img)
        
        key = cv2.waitKey(1) & 0xFF
        if key == 13: # 13 é o código da tecla ENTER
            # AGORA SIM ELE ATIVA DE FATO E CORTA O FUNDO!
            if dominio_armado == "GOJO": ativar_dominio_congelado(img, cap, dominio_armado, som_gojo, bg_gojo, "VAZIO INCOMENSURAVEL", (255, 255, 255))
            elif dominio_armado == "SUKUNA": ativar_dominio_congelado(img, cap, dominio_armado, som_sukuna, bg_sukuna, "SANTUARIO MALEVOLENTE", (0, 0, 255))
            elif dominio_armado == "FLECHA_FOGO": ativar_dominio_congelado(img, cap, dominio_armado, som_flecha, bg_flecha, "FLECHA DE FOGO (KAMINO)", (0, 165, 255))
            elif dominio_armado == "SIX_SEVEN": ativar_dominio_congelado(img, cap, dominio_armado, som_sixseven, bg_sixseven, "MODO: SIX SEVEN (67)", (255, 255, 0))
            elif dominio_armado == "MAHITO": ativar_dominio_congelado(img, cap, dominio_armado, som_mahito, bg_mahito, "AUTO-ENCARNACAO DA PERFEICAO", (255, 0, 255))
            
            # Reseta tudo depois de voltar para a vida real
            dominio_armado = None
            gesto_candidato = "NENHUM"
            frames_confirmacao = 0
            gesto_anterior = "NENHUM"
        ...
        continue # Pula o resto da leitura enquanto espera sua resposta

    # =========================================================
    # LEITURA NORMAL: PROCURANDO JUTSUS
    # =========================================================
    ...
    if hands:
        ...
        if frames_confirmacao >= LIMITE_FRAMES:
            ...
            if gesto_validado != "NENHUM" and gesto_validado != gesto_anterior:
                # EM VEZ DE ATIVAR DIRETO, ELE "ARMA" O SISTEMA
                dominio_armado = gesto_validado
            else:
    ...
    # --- HUD VISUALIZADOR DE MENTE DA IA ---
